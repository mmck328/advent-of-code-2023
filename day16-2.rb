...
candidates = []
(0...field.size).each do |x|
  candidates << Beam.new(x, 0, Direction::SOUTH)
  candidates << Beam.new(x, field[0].size - 1, Direction::NORTH)
end
(0...field[0].size).each do |y|
  candidates << Beam.new(0, y, Direction::EAST)
  candidates << Beam.new(field.size - 1, y, Direction::WEST)
end
